...
2. No systems are deployed into Qolty environments without approval of the Qolty CTO.
3. All changes to production systems, network devices, and firewalls are approved by the Qolty CTO before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Qolty's Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Qolty utilizes development and staging environments that mirror production to assure proper function.
11. Qolty also deploys environments locally using Vagrant to assure functionality before moving to staging or production.
12. All formal change requests require unique ID and authentication.
13. Qolty uses the [Security Technical Implementation Guides (STIGs)](http://iase.disa.mil/stigs/) published by the Defense Information Systems Agency as a baseline for hardening systems.
...
1. Before provisioning any systems, ops team members must file a request in the Qolty Quality Management System.
...
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Salt states.
   * These Salt states have been approved by the CTO, or an authorized delegate of the CTO, to be in accordance with all Qolty policies, including setting appropriate:
...
1. Qolty uses automated tooling to ensure systems are up-to-date with the latest security patches.
...
5. If the feature or defect interacts with ePHI, or controls access to data potentially containing ePHI, the code changes must be reviewed by two members of Qolty's Blue Team (software security team) before the feature is marked as complete.
   * The Blue Team members will provide a security analysis of features to ensure they satisfy Qolty's compliance and security commitments.
   ...
   * Blue Team members are required to undergo annual training on identifying the most common software vulnerabilities and will receive ongoing training on Qolty's compliance and security requirements.
